...
export default function Mobile() {
    ...
    return (
        <section className="w-full py-10 text-[#0B1D3A] relative font-['Outfit'] overflow-hidden" style={{ background: 'linear-gradient(135deg, #FFFFFF 0%, #F8FAFD 50%, #EEF4FF 100%)' }}>
            ...
            <div className="w-full px-5 relative z-10">
                    ...
                    {data.tabs.map(tab => (
                        <button
                            ...
                            className={`flex flex-col items-center justify-center gap-2 px-3 py-4 rounded text-[12px] font-bold transition-all duration-300 border relative overflow-hidden ${
                                activeTab === tab.id
                                    ? 'bg-[#0B1D3A]/[0.06] border-[#0B1D3A]/20 text-[#0B1D3A] shadow-[0_0_20px_rgba(11,29,58,0.08)]'
                                    : 'bg-[#0B1D3A]/[0.02] border-[#0B1D3A]/[0.06] text-[#7B8DAA]'
                            }`}
                        >
                            {activeTab === tab.id && (
                                <motion.div 
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        className="bg-white/80 border border-[#0B1D3A]/10 rounded overflow-hidden backdrop-blur-xl shadow-[0_20px_40px_-10px_rgba(11,29,58,0.12)] relative"
                    >
                        <div 
                        ...
                        <div className="p-6 flex flex-col gap-8 relative z-10">
                            <div className="flex flex-col">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <h3 className="text-[20px] font-bold leading-tight text-[#0B1D3A]">{activeContent.title}</h3>
                                </div>
                                <p className="text-[14.5px] text-[#475569] leading-relaxed mb-6 font-medium">{activeContent.desc}</p>

                                <div className="flex flex-wrap justify-center gap-5">
                                    {activeContent.sections.map((section: { heading: string; items: string[] }, idx: number) => (
                                        <div key={idx} className="flex-1 min-w-[250px] max-w-full bg-[#0B1D3A]/[0.03] rounded p-5 border border-[#0B1D3A]/[0.08]">
                                            ...
                                            <ul className="flex flex-col gap-3.5">
                                                {section.items.map((item: string, i: number) => (
                                                    <li key={i} className="flex items-start gap-3 text-[14px] text-[#334155]">
                                                        <div className="w-4 h-4 rounded-full flex items-center justify-center shrink-0 mt-0.5 bg-[#0B1D3A]/10" style={{ color: activeColor }}>
                            ...
                            <div className="flex flex-col gap-4 border-t border-[#0B1D3A]/[0.08] pt-6">
                                {activeContent.journey && (
                                    <div className="bg-[#0B1D3A]/[0.04] rounded p-5 border border-[#0B1D3A]/10">
                                        ...
                                        <div className="flex flex-col gap-2.5">
                                            {activeContent.journey.split(' → ').map((step: string, i: number, arr: string[]) => (
                                                <div key={i} className="flex items-center gap-3">
                                                    <div className="w-6 h-6 rounded-full border border-[#0B1D3A]/15 flex items-center justify-center text-[10px] font-bold text-[#7B8DAA]">{i + 1}</div>
                                    ...
                                    <div className="bg-[#0B1D3A]/[0.04] rounded p-5 border border-[#0B1D3A]/10">
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                            {activeContent.evaluateBasedOn.split(' · ').map((tag: string, i: number) => (
                                                <span key={i} className="px-2.5 py-1.5 rounded bg-[#0B1D3A]/[0.04] border border-[#0B1D3A]/10 text-[12px] font-medium text-[#475569]">{tag}</span>
                                            ))}
                                        </div>
                                    </div>
                                )}

                                <div className="bg-gradient-to-b from-[#0B1D3A]/[0.05] to-[#0B1D3A]/[0.02] rounded p-6 border border-[#0B1D3A]/10 shadow-[0_10px_20px_rgba(11,29,58,0.06)] mt-2">
                                    <p className="text-[14px] font-medium italic mb-5 text-center text-[#0B1D3A]/85 leading-snug">"{activeContent.footerText}"</p>
                                            ...
                                            <button
                                                ...
                                                className={`w-full py-3.5 px-4 rounded text-[13.5px] font-bold transition-all duration-300 flex items-center justify-center gap-2 ${
                                                    idx === 0
                                                        ...
                                                        : 'bg-[#0B1D3A]/5 text-[#0B1D3A] border border-[#0B1D3A]/15 active:scale-[0.98]'
                                                }`}
