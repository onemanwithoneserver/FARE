        ...
        >
          ...
          >
            {data.headline.split(" ").map((word, i, arr) => (
              <span
                key={i}
                className={i >= arr.length - 3 ? "text-[#C99A2E]" : ""}
              >
                {word}{" "}
              </span>
            ))}
            ...
            {data.tabs.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-[18px] font-bold transition-colors duration-300 ${isActive ? "text-white" : "text-[#0B1D3A] group-hover:text-[#C99A2E]"}`}
                  >
                    {tab.label}
                  </span>
                  {isActive && (
          ...
          <div className="flex-1 w-full bg-white border border-[#0B1D3A]/[0.06] rounded p-10 lg:p-14 flex flex-col relative overflow-hidden shadow-[0_10px_40px_-10px_rgba(11,29,58,0.05)]">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <h3
                  className="text-[32px] lg:text-[40px] font-black mb-6 leading-tight"
                  style={{ color: NAVY }}
                >
                  {activeTabData.title}
                </h3>
