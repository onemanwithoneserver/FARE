...
import { Move3d, Compass, Lightbulb, Workflow, Network } from "lucide-react";
import { getData } from "./data";
import { useLanguage } from "../../../context/LanguageContext";

const ICONS = [Compass, Move3d, Lightbulb, Workflow, Network];
const GRADIENTS = [
  "from-[#FBBF24] to-[#D97706]", "from-[#38BDF8] to-[#0284C7]", "from-[#34D399] to-[#059669]", 
  "from-[#F472B6] to-[#DB2777]", "from-[#A78BFA] to-[#7C3AED]"
];
    ...
    <section className="w-full bg-[#F8FAFD] py-24 px-10 font-['Outfit'] relative overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10">
        <motion.div
          ...
          className="mb-16"
        >
          <h2 className="text-4xl lg:text-[2.75rem] font-black text-[#0B1D3A] text-center tracking-tight leading-tight">
            {data.title}
          </h2>
        </motion.div>
          ...
          {data.outcomes.map((o, i) => {
            ...
            return (
              <motion.div
                key={i}
                variants={item}
                className="bg-white p-8 rounded-2xl border border-[#E2E8F0]/80 shadow-[0_4px_16px_rgba(11,29,58,0.02)] hover:shadow-[0_12px_32px_rgba(11,29,58,0.06)] hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden"
              >
        ...
        >
          <p className="text-[19px] font-bold text-[#C99A2E] bg-white inline-block px-8 py-4 rounded-xl shadow-sm border border-[#E2E8F0]/80">
            "{data.quote}"
          </p>
        </motion.div>
      </div>
