import type { Language } from '../../context/LanguageContext';

export const dataEn = {
    title: "Section 4 - Explore FARE Custom RE LMS",
    headline: "See how FARE works for your organisation.",
    subtitle: "Use the tabs to explore how FARE can fit into your learning ecosystem.",
    journeyLabel: "Journey",
    evaluateLabel: "Evaluate Based On",
    tabs: [
        { id: "tab1", title: "Project Knowledge" },
        { id: "tab2", title: "Knowledge Bank" },
        { id: "tab3", title: "Mocks" },
        { id: "tab4", title: "Trainers" }
    ],
    tabContent: {
        tab1: {
            title: "Put your project knowledge into a structured learning journey.",
            desc: "Instead of distributing project information across PDFs, presentations, WhatsApp messages and training sessions, create one structured learning experience.",
            sections: [
                {
                    heading: "Project Overview",
                    items: ["Project story, positioning and key differentiators."]
                },
                {
                    heading: "Location",
                    items: ["Connectivity, neighbourhood, infrastructure and location advantages."]
                },
                {
                    heading: "Product",
                    items: ["Configurations, specifications, layouts and key features."]
                },
                {
                    heading: "Amenities & Lifestyle",
                    items: ["Amenities, community features and lifestyle proposition."]
                },
                {
                    heading: "Pricing & Payment Plans",
                    items: ["Pricing structure, payment plans, offers and commercial details."]
                },
                {
                    heading: "Approvals & Key Information",
                    items: ["Important approvals, specifications and project information."]
                }
            ],
            journey: "Learn → Complete → Quiz → Track",
            footerText: "Measure project knowledge instead of assuming everyone knows it.",
            buttons: ["Build Project Course"]
        },
        tab2: {
            title: "Keep your Sales & CP teams connected with your project.",
            desc: "Create a dedicated Knowledge Bank to continuously test and reinforce project knowledge.",
            sections: [
                {
                    heading: "Use it for:",
                    items: [
                        "New project onboarding",
                        "New Sales Associates",
                        "Channel Partner onboarding",
                        "Project launch preparation",
                        "Regular knowledge checks",
                        "Product updates",
                        "Location knowledge",
                        "Pricing & payment plan updates"
                    ]
                },
                {
                    heading: "Measure:",
                    items: [
                        "What they know",
                        "What they don't know",
                        "What needs reinforcement"
                    ]
                }
            ],
            footerText: "Don't assume your team knows the project. Measure it.",
            buttons: ["Explore Knowledge Bank"]
        },
        tab3: {
            title: "Knowing what to say is different from knowing how to say it.",
            desc: "Give your teams opportunities to practise real sales situations.",
            sections: [
                {
                    heading: "Mock scenarios can include:",
                    items: [
                        "Pre-Sales: Opening the conversation, creating impact, qualifying the lead",
                        "Need Analysis: Asking the right questions, understanding customer requirements, identifying buying intent",
                        "Sales: Project presentation, product recommendation, objection handling, follow-up",
                        "Channel Partners: Project pitch, CP engagement, handling CP questions, project positioning"
                    ]
                }
            ],
            journey: "Scenario → Mock → Feedback → Improvement",
            footerText: "Give your team the confidence they need before they face the customer.",
            buttons: ["Explore Mocks"]
        },
        tab4: {
            title: "Need specialised skills? Bring the right trainer in.",
            desc: "Find trainers based on your specific requirement rather than searching for a generic trainer.",
            sections: [
                {
                    heading: "Discover trainers by:",
                    items: [
                        "Expertise: Sales, Pre-Sales, Communication, Leadership, Digital, Customer Experience, Marketing",
                        "RE Segment: Residential, Commercial, Plotted Development, Generic Real Estate",
                        "Format: Workshops, Half-Day, Full-Day, Mocks, Bootcamps, Coaching, Mentoring",
                        "Delivery: Online, Offline, Blended"
                    ]
                }
            ],
            evaluateBasedOn: "Expertise · RE Segment · Format · Delivery",
            journey: "Discover → Shortlist → Compare → Request",
            footerText: "Bring the right trainer into your learning program when your team needs specialised skills.",
            buttons: ["Explore Trainer Directory"]
        }
    }
};

export const dataTe = {
    title: "సెక్షన్ 4 — FARE కస్టమ్ RE LMS అన్వేషించండి",
    headline: "FARE మీ ఆర్గనైజేషన్ కోసం ఎలా పనిచేస్తుందో చూడండి.",
    subtitle: "FARE మీ లెర్నింగ్ ఎకోసిస్టమ్‌లో ఎలా సరిపోతుందో తెలుసుకోవడానికి ట్యాబ్‌లను ఉపయోగించండి.",
    journeyLabel: "జర్నీ",
    evaluateLabel: "ఆధారంగా మూల్యాంకనం",
    tabs: [
        { id: "tab1", title: "Project Knowledge" },
        { id: "tab2", title: "Knowledge Bank" },
        { id: "tab3", title: "Mocks" },
        { id: "tab4", title: "Trainers" }
    ],
    tabContent: {
        tab1: {
            title: "మీ ప్రాజెక్ట్ నాలెడ్జ్‌ను ఒక స్ట్రక్చర్డ్ లెర్నింగ్ జర్నీలో ఉంచండి.",
            desc: "ప్రాజెక్ట్ సమాచారాన్ని PDFలు, ప్రెజెంటేషన్లు, WhatsApp మెసేజ్‌లు మరియు ట్రైనింగ్ సెషన్స్‌లో పంచే బదులు, ఒకే స్ట్రక్చర్డ్ లెర్నింగ్ అనుభవాన్ని సృష్టించండి.",
            sections: [
                {
                    heading: "ప్రాజెక్ట్ అవలోకనం",
                    items: ["ప్రాజెక్ట్ స్టోరీ, పొజిషనింగ్ మరియు ముఖ్యమైన డిఫరెన్షియేటర్లు."]
                },
                {
                    heading: "లొకేషన్",
                    items: ["కనెక్టివిటీ, ఇరుగుపొరుగు, ఇన్‌ఫ్రాస్ట్రక్చర్ మరియు లొకేషన్ అడ్వాంటేజెస్."]
                },
                {
                    heading: "ప్రొడక్ట్",
                    items: ["కాన్ఫిగరేషన్లు, స్పెసిఫికేషన్లు, లేఅవుట్‌లు మరియు ముఖ్య లక్షణాలు."]
                },
                {
                    heading: "అమెనిటీస్ & లైఫ్‌స్టైల్",
                    items: ["అమెనిటీస్, కమ్యూనిటీ ఫీచర్లు మరియు లైఫ్‌స్టైల్ ప్రొపోజిషన్."]
                },
                {
                    heading: "ప్రైసింగ్ & పేమెంట్ ప్లాన్స్",
                    items: ["ప్రైసింగ్ స్ట్రక్చర్, పేమెంట్ ప్లాన్స్, ఆఫర్లు మరియు కమర్షియల్ వివరాలు."]
                },
                {
                    heading: "అప్రూవల్స్ & ముఖ్య సమాచారం",
                    items: ["ముఖ్యమైన అప్రూవల్స్, స్పెసిఫికేషన్లు మరియు ప్రాజెక్ట్ సమాచారం."]
                }
            ],
            journey: "Learn → Complete → Quiz → Track",
            footerText: "అందరికీ తెలుసని ఊహించే బదులు ప్రాజెక్ట్ నాలెడ్జ్‌ను కొలవండి.",
            buttons: ["ప్రాజెక్ట్ కోర్సును నిర్మించండి"]
        },
        tab2: {
            title: "మీ ప్రాజెక్ట్‌తో మీ సేల్స్ & CP టీమ్‌లను కనెక్ట్ చేసి ఉంచండి.",
            desc: "ప్రాజెక్ట్ నాలెడ్జ్‌ను నిరంతరం పరీక్షించడానికి మరియు బలోపేతం చేయడానికి ఒక ప్రత్యేకమైన నాలెడ్జ్ బ్యాంక్‌ను సృష్టించండి.",
            sections: [
                {
                    heading: "దీని కోసం ఉపయోగించండి:",
                    items: [
                        "కొత్త ప్రాజెక్ట్ ఆన్‌బోర్డింగ్",
                        "కొత్త సేల్స్ అసోసియేట్స్",
                        "ఛానల్ పార్ట్‌నర్ ఆన్‌బోర్డింగ్",
                        "ప్రాజెక్ట్ లాంచ్ ప్రిపరేషన్",
                        "రెగ్యులర్ నాలెడ్జ్ చెక్స్",
                        "ప్రొడక్ట్ అప్‌డేట్స్",
                        "లొకేషన్ నాలెడ్జ్",
                        "ప్రైసింగ్ & పేమెంట్ ప్లాన్ అప్‌డేట్స్"
                    ]
                },
                {
                    heading: "కొలవండి:",
                    items: [
                        "వారికి ఏమి తెలుసు",
                        "వారికి ఏమి తెలియదు",
                        "ఏమి బలోపేతం చేయాలి"
                    ]
                }
            ],
            footerText: "మీ టీమ్‌కు ప్రాజెక్ట్ గురించి తెలుసని ఊహించకండి. దాన్ని కొలవండి.",
            buttons: ["నాలెడ్జ్ బ్యాంక్‌ను అన్వేషించండి"]
        },
        tab3: {
            title: "ఏమి చెప్పాలో తెలుసుకోవడం అనేది ఎలా చెప్పాలో తెలుసుకోవడం కంటే భిన్నమైనది.",
            desc: "నిజమైన సేల్స్ పరిస్థితులను ప్రాక్టీస్ చేయడానికి మీ టీమ్‌లకు అవకాశాలు ఇవ్వండి.",
            sections: [
                {
                    heading: "మాక్ దృశ్యాలు ఇలా ఉండవచ్చు:",
                    items: [
                        "ప్రీ-సేల్స్: సంభాషణను ప్రారంభించడం, ప్రభావం చూపడం, లీడ్‌ను అర్హత చేయడం",
                        "నీడ్ అనాలిసిస్: సరైన ప్రశ్నలు అడగడం, కస్టమర్ అవసరాలను అర్థం చేసుకోవడం, కొనుగోలు ఉద్దేశ్యాన్ని గుర్తించడం",
                        "సేల్స్: ప్రాజెక్ట్ ప్రెజెంటేషన్, ప్రొడక్ట్ రికమెండేషన్, అబ్జెక్షన్ హ్యాండ్లింగ్, ఫాలో-అప్",
                        "ఛానల్ పార్ట్‌నర్స్: ప్రాజెక్ట్ పిచ్, CP ఎంగేజ్‌మెంట్, CP ప్రశ్నలను హ్యాండిల్ చేయడం, ప్రాజెక్ట్ పొజిషనింగ్"
                    ]
                }
            ],
            journey: "Scenario → Mock → Feedback → Improvement",
            footerText: "కస్టమర్‌ను ఎదుర్కోవడానికి ముందే మీ టీమ్‌కు అవసరమైన కాన్ఫిడెన్స్ ఇవ్వండి.",
            buttons: ["మాక్స్‌ను అన్వేషించండి"]
        },
        tab4: {
            title: "ప్రత్యేక స్కిల్స్ అవసరమా? సరైన ట్రైనర్‌ను తీసుకురండి.",
            desc: "సాధారణ ట్రైనర్ కోసం వెతికే బదులు మీ నిర్దిష్ట అవసరాల ఆధారంగా ట్రైనర్‌లను కనుగొనండి.",
            sections: [
                {
                    heading: "ట్రైనర్‌లను ఇలా అన్వేషించండి:",
                    items: [
                        "ఎక్స్‌పర్టీస్: సేల్స్, ప్రీ-సేల్స్, కమ్యూనికేషన్, లీడర్‌షిప్, డిజిటల్, కస్టమర్ ఎక్స్‌పీరియన్స్, మార్కెటింగ్",
                        "RE సెగ్మెంట్: రెసిడెన్షియల్, కమర్షియల్, ప్లాటెడ్ డెవలప్‌మెంట్, జెనరిక్ రియల్ ఎస్టేట్",
                        "ఫార్మాట్: వర్క్‌షాప్స్, హాఫ్-డే, ఫుల్-డే, మాక్స్, బూట్‌క్యాంప్స్, కోచింగ్, మెంటరింగ్",
                        "డెలివరీ: ఆన్‌లైన్, ఆఫ్‌లైన్, బ్లెండెడ్"
                    ]
                }
            ],
            evaluateBasedOn: "ఎక్స్‌పర్టీస్ · RE సెగ్మెంట్ · ఫార్మాట్ · డెలివరీ",
            journey: "Discover → Shortlist → Compare → Request",
            footerText: "మీ టీమ్‌కు ప్రత్యేక స్కిల్స్ అవసరమైనప్పుడు సరైన ట్రైనర్‌ను మీ లెర్నింగ్ ప్రోగ్రామ్‌లోకి తీసుకురండి.",
            buttons: ["ట్రైనర్ డైరెక్టరీని అన్వేషించండి"]
        }
    }
};

export const getData = (lang: Language = 'en') => lang === 'te' ? dataTe : dataEn;
export const data = dataEn;
